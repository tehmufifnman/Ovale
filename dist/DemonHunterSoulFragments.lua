local OVALE, Ovale = ...
require(OVALE, Ovale, "DemonHunterSoulFragments", { "./playerGUID" }, function(__exports, __playerGUID)
...
local OvaleDemonHunterSoulFragments = __class()
...
function OvaleDemonHunterSoulFragments:COMBAT_LOG_EVENT_UNFILTERED(event, _, subtype, _, sourceGUID, _, _, _, _, _, _, _, spellID, spellName)
    if sourceGUID == __playerGUID.me then
...
statePrototype.SoulFragments = function(state, atTime)
    for k, v in spairs(OvaleDemonHunterSoulFragments.soul_fragments, function(t, a, b)
