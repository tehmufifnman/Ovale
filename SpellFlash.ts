...
let colorCd = {  }
let colorInterrupt = {  }
let FLASH_COLOR = { main: colorMain, cd: colorCd, shortcd: colorCd }
let COLORTABLE = { aqua: { r: 0, g: 1, b: 1 }, blue: { r: 0, g: 0, b: 1 }, gray: { r: 0.5, g: 0.5, b: 0.5 }, green: { r: 0.1, g: 1, b: 0.1 }, orange: { r: 1, g: 0.5, b: 0.25 }, pink: { r: 0.9, g: 0.4, b: 0.4 }, purple: { r: 1, g: 0, b: 1 }, red: { r: 1, g: 0.1, b: 0.1 }, white: { r: 1, g: 1, b: 1 }, yellow: { r: 1, g: 1, b: 0 } }
{
    let defaultDB = { spellFlash: { brightness: 1, enabled: true, hasHostileTarget: false, hasTarget: false, hideInVehicle: false, inCombat: false, size: 2.4, threshold: 500, colorMain: { r: 1, g: 1, b: 1 }, colorShortCd: { r: 1, g: 1, b: 0 }, colorCd: { r: 1, g: 1, b: 0 }, colorInterrupt: { r: 0, g: 1, b: 1 } } }
    let options = { spellFlash: { type: "group", name: "SpellFlash", disabled: function () {
    ...
    }, args: { enabled: { order: 10, type: "toggle", name: L["Enabled"], desc: L["Flash spells on action bars when they are ready to be cast. Requires SpellFlashCore."], width: "full" }, inCombat: { order: 10, type: "toggle", name: L["En combat uniquement"], disabled: function () {
        return !SpellFlashCore || !Ovale.db.profile.apparence.spellFlash.enabled;
    } }, hasTarget: { order: 20, type: "toggle", name: L["Si cible uniquement"], disabled: function () {
        return !SpellFlashCore || !Ovale.db.profile.apparence.spellFlash.enabled;
    } }, hasHostileTarget: { order: 30, type: "toggle", name: L["Cacher si cible amicale ou morte"], disabled: function () {
        return !SpellFlashCore || !Ovale.db.profile.apparence.spellFlash.enabled;
    } }, hideInVehicle: { order: 40, type: "toggle", name: L["Cacher dans les véhicules"], disabled: function () {
        return !SpellFlashCore || !Ovale.db.profile.apparence.spellFlash.enabled;
    } }, brightness: { order: 50, type: "range", name: L["Flash brightness"], min: 0, max: 1, bigStep: 0.01, isPercent: true, disabled: function () {
        return !SpellFlashCore || !Ovale.db.profile.apparence.spellFlash.enabled;
    } }, size: { order: 60, type: "range", name: L["Flash size"], min: 0, max: 3, bigStep: 0.01, isPercent: true, disabled: function () {
        return !SpellFlashCore || !Ovale.db.profile.apparence.spellFlash.enabled;
    } }, threshold: { order: 70, type: "range", name: L["Flash threshold"], desc: L["Time (in milliseconds) to begin flashing the spell to use before it is ready."], min: 0, max: 1000, step: 1, bigStep: 50, disabled: function () {
        return !SpellFlashCore || !Ovale.db.profile.apparence.spellFlash.enabled;
    } }, colors: { order: 80, type: "group", name: L["Colors"], inline: true, disabled: function () {
        return !SpellFlashCore || !Ovale.db.profile.apparence.spellFlash.enabled;
    }, get: function (info) {
        ...
        color.b = b;
        OvaleOptions.SendMessage("Ovale_OptionChanged");
    }, args: { colorMain: { order: 10, type: "color", name: L["Main attack"], hasAlpha: false }, colorCd: { order: 20, type: "color", name: L["Long cooldown abilities"], hasAlpha: false }, colorShortCd: { order: 30, type: "color", name: L["Short cooldown abilities"], hasAlpha: false }, colorInterrupt: { order: 40, type: "color", name: L["Interrupts"], hasAlpha: false } } } } } }
